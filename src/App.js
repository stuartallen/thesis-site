...
const equations = {
  ...
  Derivative:
    [
      { latex: String.raw`\frac{d}{d\pi_1}`, explanation: 'The derivative with respect to the weight of the first cluster'},
      { latex: String.raw`\sum_{i=1}^n`, explanation: 'The sum of n data points'},
      { latex: String.raw`log(`},
      { latex: String.raw`\pi_1`, explanation: 'The likelihood assosciated with cluster 1'},
      ...
      { latex: String.raw`\pi_2`, explanation: 'The likelihood assosciated with cluster 2'},
  ...
  SoftAssignments: [
    { latex: String.raw`\gamma_x = `, explanation: 'Let\'s use gamma to represent the soft assignments'},
    ...
    <div className="App" style={{backgroundColor: useColor('light')}}>
      ...
      <div className='explanation'>
        <p>
          ...
          accomplished with <strong>Gaussian mixture models</strong>.
        </p>
      </div>
      ...
      <div className='explanation'>
        ...
        mean and variance. The mean of a distribution describes the point at which the largest values of the function are centered around, and the variance
      ...
      <div className='explanation'>
        ...
        sighting activity of several. A mixture of Gaussians can be made from a weighted sum of two or more distributions. This is a Gaussian mixture model. Applying a weight to each
        ...
        individual distributions increases the flexibility of our mixture. Observe a Gaussian mixture model in the one dimensional case:
      </div>
      ...
      <div className='explanation' id='densityEstimator'>
        <p>
          ...
          Gaussian mixture models to find a good density estimator of the likelihood of seeing a mountain lion at any point on our map.
        </p>
      </div>

      <h3 id="logLikelihood">Evaluating the Fit of a Gaussian Mixture Model</h3>

      <div className='explanation'>
        <p>
          The fit of a Gaussian mixture model can be measured in many ways, but a popular choice is the <strong>log likelihood expression</strong>. The log likelihood is simply
      ...
      <div className='explanation'>
        Holding the number of individual distribution constant a mixture with a higher log likelihood than another is usually a better representation of the data. However, there
      ...
      <div className='explanation'>
        ...
        can be responsible for less of the data until it only represents one data point. In this manner if we optimize with respect to number of individual distributions, we learn nothing about
        the distribution underlying the data. Instead, there is typically a point where increasing the number of individual distributions presents diminishing returns for the log likelihood. Choosing 
        a value where the diminishing returns begin typically yields a reasonable number of individual distributions to represent the data. This shows that just because a Gaussian mixture model
        has more individual distributions, it is not necessarily a better representation of a data set.
      </div>

      <h3>Check your Understanding</h3>
      <div className='explanation'>
        The following questions contain visualizations of Gaussian mixture model distributions and data in a plane. Given what you've just learned about Gaussian
      ...
      <Question 
        ...
        options={['Left','Right']}
        expandedOptions={
          ['Correct. A non-cirular distribution does a better job representing how spread these data points are.', 
      ...
      <div className='explanation'>
        ...
        all the points under a multidimensional function along a line. Our multidimensional function is our multivariate Gaussian mixture model. This kind of 
      ...
      <h3 id='MLE'>Why Gaussian Mixture Models Cannot Be Learned Directly</h3>

      <div className='explanation'>
        <p>
          As we are only given the locations of mountain lion sightings, we must find a Gaussian mixture model that best represents these sightings in order to
          ...
          &nbsp;(MLE). Many machine learning problems can be solved through directly maximizing the <a href='#logLikelihood'>log likelihood expression</a>. The log likelihood is a useful expression as it is a 
          ...
          many cases a closed form solution is possible. Answer the following question to observe why this is not the case for optimizing Gaussian mixture models.
        </p>
      </div>

      <Question 
        question={"In order to directly solve a Gaussian mixture model in a one dimensional case with two clusters, this is just one of the derivatives we must find the roots of. Which terms will appear in the resulting expression after the derivative operator is applied?"}
      ...
      <div className='explanation'>
        <p>
          In the case of directly optimizing Gaussian mixture models our log likelihood expression yields a summation dependent on every parameter within a logarithm. Our optimization then leads to not only a system of nonlinear equations, but also 
          has no closed form solution.&nbsp;<a href='#bishop'>[1]</a> This means we cannot build the best density estimator as some operation of our data set, nor can we know the best Gaussian mixture model that 
      ...
      <div className='explanation'>
        <p>
          We have shown why it is impossible to directly optimize Gaussian mixture models with <a href='#MLE'>MLE</a>, however, we can use the Expectation Maximization (EM) algorithm to 
      ...
      <div className='explanation'>
        In order to take advantage of the EM algorithm, we must introduce our latent variable. Suppose we have a Gaussian mixture model with three individual 
        Gaussian distributions as the density estimator of the likelihood of seeing mountain lion at each point on the map. Our latent variable will be the likelihood that each individual sighting
      ...
      <div className='explanation'>
        To fit the EM algorithm to the problem of optimizing Gaussian mixture models, it is important to make the realization that given the initial distribution parameters of our
        Guassian mixture model (the means, covariance matrices, and weights assosciated with each distribution), it is trivial to update the probability assignment for each data point. This is known as the expectation step.
      </div>
      <Question 
      ...
      <div className='explanation'>
        We must also make the realization that it is trivial to calculate an update of our Guassian mixture model's distribution parameters given our probability assignments. This is called the maximization step. 
        Our distribution parameters are updated according to the formulas below:
      </div>
      ...
      <div className='explanation'>
        <p>
          As aforementioned, the process of switching between the expectation and maximization step can continue until a local solution is achieved. A local solution does not guarantee a great 
          solution and it is therefore important to run the EM algorithm with many different initial distribution parameters. Different initialization strategies for Gaussian mixture models will be discussed later 
      ...
      <div className='explanation'>
        <ol>
          <li>Use the <a href='#EM'>EM algorithm</a> to learn a Gaussian mixture model, which acts as a <a href='#densityEstimator'>density estimator</a> of our data set (positions of mountain lion sightings).</li>
          ...
          <li>Approximate the <a href='#lineIntegral'>line integral</a> along each path using Reimann sum approximation. This is our likelihood of seeing a mountain lion on this path.</li>
      ...
      <div className='explanation'>
        <p>
          ...
          assignments of each data point to an individual distribution. That is the probability for each point that it came from a particular individual distribution. Soft assignments
          ...
          algorithms take covariance into account when forming clusters. For example, K-means is one such algorithm that does not take covariance into account when clustering data.
        </p>
      </div>
      ...
      <Question
        ...
        visual={'kmeans'}
        options={['Left', 'Right']}
        expandedOptions={['Correct. K-means is not robust to covariance and performs worse at the edges of non-circular clusters.','Incorrect. K-means is not robust to covariance and would struggle to exactly label the edges of non-circular clusters.']}
        correctness={[true, false]}
      />
      ...
      <div className='explanation'>
        Without safegaurding, it is possible a Gaussian mixture model can contain a singularity. This is where one of the normal distributions begins to center around one datapoint and
        the covariance matrix entries approach infinitely small values. Even if this may increase the likelihood expression of of a data set, it is likely not
      ...
      <div className='explanation'>
        ...
        and weight for each normal distribution can be initialized as the identity matrix and equal weighting, or a psuedorandom value. For mean initializations, it is possible
        to choose random points within the range of the data set or random points in the data set itself. It is also possible to choose an existing point, weight the likelihood of choosing
        ...
        of means generation will likely generate better results than others, as the means are unlikely to be initialized near each other.
      </div>
